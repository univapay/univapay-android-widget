...
/**
 * Checkout configuration that creates a recurring token to process a transaction.
 * Requires the merchant to be verified.
 */
class RecurringTokenCheckout: CheckoutConfiguration<PaymentSettings> {

    constructor(): super()

    constructor(settings: PaymentSettings): super(settings)

    constructor(settings: PaymentSettings, processFullPayment: Boolean): super(settings, processFullPayment)

    override val transactionTokenType: TransactionTokenType
        get() = TransactionTokenType.RECURRING
    ...
    override fun process(email: String, paymentData: PaymentData, univapay: UnivapaySDK, context: Context, arguments: CheckoutArguments, rememberCard: Boolean) {
        ...
        fun createWithCVVRetry(paymentData: PaymentData){
            val builder = getTokenBuilder(univapay, email, paymentData, transactionTokenType, arguments.metadata)
                    .withUsageLimit(arguments.recurringTokenUsageLimit)

            arguments.customerId?.let {id->
                if(rememberCard){
                    builder.withCustomerId(UnivapayCustomerId(id))
                }
            }
            builder
                    .build()
    ...
    override fun writeToParcel(parcel: Parcel, i: Int) {
        ...
        parcel.writeByte((if(processFullPayment) 1 else 0).toByte())
    }


    companion object {
        @JvmField val CREATOR: Parcelable.Creator<RecurringTokenCheckout> = object: Parcelable.Creator<RecurringTokenCheckout>{
            override fun createFromParcel(parcel: Parcel): RecurringTokenCheckout {
                val settings = parcel.readParcelable<PaymentSettings>(PaymentSettings::class.java.classLoader)
            ...
            override fun newArray(i: Int): Array<RecurringTokenCheckout?> {
